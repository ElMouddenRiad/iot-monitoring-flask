...
def create_app():
    ...
    @app.route("/health")
    def health_check():
        return jsonify({"service": "device-management", "status": "healthy"}), 200
    ...
    def predict_device():
        try:
            ...
            if not device:
                return jsonify({"error": f"Aucun appareil trouve pour la mac {mac}"}), 404
            ...
            if not responses:
                return jsonify({"error": "Aucune reponse de l'API Open-Meteo"}), 502
...
if __name__ == "__main__":
    socketio.run(
        app,
        host="0.0.0.0",
        port=int(app.config.get("PORT", 5002)),
